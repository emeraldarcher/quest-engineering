...
Quest Engineering Worker (separate Bun/TypeScript component)
        ↓
HarnessRegistry (PiHarness + interactive-first AntigravityHarness)
        ↓
TerminalSessionBackend (HerdrTerminalBackend)
```

The control plane hosts Worker Protocol v7 and `workers/bun` is the filesystem authority. Live session identity, local Herdr attachment, and operational HumanAttention are documented in [`docs/live-execution-sessions.md`](docs/live-execution-sessions.md). Product Workspaces are logical and path-free; long-lived Workers discover explicitly authorized source repositories and provision one durable isolated Git worktree per Run before model execution. Workspace bindings reconcile independently when a Worker connects: a binding to a Product Workspace that no longer exists is retired without taking the Worker offline or deleting retained worktrees, dispatch state, or harness-native sessions. After successful Runtime completion, the Worker safely finalizes and pushes Git changes while Phoenix creates and centrally reconciles one same-repository GitHub Pull Request. Only an exact human-merged PR completes the Quest; publishing retry, explicit Run Again, and safe worktree cleanup remain separate operations. The Svelte/Pixi/Tauri client presents this truthful lifecycle as a dense Mini Medieval fantasy management town with deterministic Members, work orders, delivery markers, and accessible DOM management windows.

Planning Classes, immutable Markdown execution artifacts, exact-subject verdicts, and independent plan/code remediation gates are documented in [`docs/spec-driven-development.md`](docs/spec-driven-development.md).
...
- `quest_engineering_server` — Phoenix/Ecto infrastructure boundary with Product Tactic persistence, immutable resolved launch snapshots, transactional runtime persistence, and the generation-fenced WebSocket Worker Protocol v7.
