    ...
    -> AgentHarness
         -> PiHarness
              -> SbxRunExecutionManager
                   -> Run-owned mixed SBX / private Git / guest Pi or Antigravity
...
`PiHarness` and `AntigravityHarness` adapt their native lifecycles without changing generic fresh/continuation selection, result envelopes, output collection, cancellation, or checkpoint/export semantics. In production each prepares the same Run-owned mixed SBX lease plus a private-lineage Git worktree, asks the lease for one authoritative `HostLaunchDescriptor`, and gives it unchanged to Herdr explicit managed launch. Herdr executes the descriptor's absolute QE/backend-owned launcher with literal argv, cwd, environment overlay, and PTY semantics. No host-visible `pi`/`agy` wrapper, shell, `PATH` lookup, or direct-host fallback participates. Each harness runtime, integration state, HOME, cache, and working directory remain inside the guest. `FakeHarness` deterministically simulates running, attention, resume, completion, interruption, failure/closure and recovery without invoking a provider.
...
On macOS, Tauri passes a private launch descriptor to a short-lived native helper opened as a Terminal document. The Product descriptor never supplies an executable. Worker/session-host and the native client share one local runtime configuration authority: `QE_HERDR_BIN`. Both require it to identify an absolute, existing executable file and canonicalize it before use; neither consults `PATH`, Homebrew locations, or `/usr/local/bin`. Missing, relative, non-executable, or otherwise unavailable configuration fails closed as `Compatible Quest Engineering Herdr runtime unavailable.`

The helper starts that locally resolved Herdr executable with `std::process::Command`: session and pane are separate literal argv values, observe mode omits `--takeover`, and no attachment value is interpolated into shell source. The Product descriptor token, Worker generation, Run/Attempt/session binding, local Herdr ownership tokens, terminal identity, and live pane identity remain independently fenced before this launch. Development and acceptance launchers must pass the same explicit `QE_HERDR_BIN` used by the Worker into the Tauri process; an interactive shell's `PATH` is never configuration. A future packaged managed/sidecar runtime can satisfy the same local resolver contract, but packaging work is intentionally outside this phase.
...
Paid-inference authorization is independent of scheduling authorization. Every non-fake initial Attempt and human recovery may prepare and attest its native process, but must then stop at a durable `needs_confirmation` gate with no prompt intent. The configured `initial` recovery epoch grants only the ordinary scheduling allowance. A separate operator `authorize-prompt` decision is required to release exactly that prepared Attempt; fake deterministic execution is provider-free and exempt. This makes an inputless Open Session probe possible before any provider request.

Product cancellation is independent of both scheduling and prompt authorization. Phoenix serializes cancellation intent with prompt authorization under the locked Run, persists exact Attempt/Action/request provenance, and only then sends the generation-fenced Worker command. A cancellation that wins first prevents later prompt authorization. A prior prompt authorization remains historical, but cannot override a later terminal cancellation. The Worker interrupts through the harness adapter, reports the canonical `execution_cancelled` fact, keeps result/control authority through Phoenix acknowledgement, retains the session and private worktree, and never creates a recovery Attempt.
...
Remote attachment is disabled. `QE_LOCAL_SESSION_ATTACH_ENABLED=true` must explicitly enable the endpoint. Phase-4 acceptance launchers must fail closed before prompt intent unless both the launcher environment and Phoenix `/api/v1/health` report it enabled, the local Tauri/macOS and patched-Herdr prerequisites validate, and an inputless descriptor/Herdr-target probe succeeds against the exact harmless pre-prompt pane. Environment text alone is not proof of effective server configuration. This is acceptance configuration only; the production default remains disabled. It additionally requires a direct loopback request, a localhost Host header, no forwarding headers, and the local Tauri client marker. The endpoint validates the exact Run, Attempt, dispatch, session, Worker and current Worker generation, then issues an on-demand descriptor with a signed token valid for 60 seconds. It never returns a reusable shell command or remote shell URL.

Tauri resolves `QE_HERDR_BIN` locally—never from Product data—then validates descriptor shape and takeover authority, confirms that the named Herdr session is already running via that exact configured executable's `session list --json`, verifies the exact terminal plus QE ownership/Worker/lineage provenance from `agent get`, and only then opens macOS Terminal. A running session opens observation-only. `--takeover` is accepted only for a session currently projected as waiting for the human. The native success callback is then recorded as local-session-opened/human-control-started. Herdr 0.9.0 exposes no separate durable attach-ack query, so QE's strongest confirmation is successful native validation plus macOS Terminal accepting the exact attach launch. Browser clients display the Worker hosting the session and honestly state that browser attachment is unavailable.
