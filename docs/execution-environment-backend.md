...
For each existing Run sandbox, the Worker validates the host Pi `openai-codex` OAuth credential through Pi's own `ModelRuntime.getAuth`. Resolution runs under Pi's interoperable auth-file lock in a same-directory shadow store; Pi alone decides expiry, performs refresh, and rotates fields, after which QE verifies that unrelated providers did not change and atomically renames the mode-preserving result. The helper allows no host request except Pi's exact token endpoint, emits only the current access token on stdout, emits only a fixed redacted failure on stderr, and receives no token in argv. SBX stores only that helper command in a sandbox-scoped dynamic custom secret with a `5m` refresh interval and replaces the placeholder only for `chatgpt.com`. Actual tokens remain in the host Pi store and SBX proxy memory; they never enter the VM, QE SQLite/PostgreSQL state, diagnostics, evidence, or retained command arguments.
...
Pi's generated model registry is harness-support evidence, not account entitlement. The profile decodes the authenticated models response and requires explicit API support, picker visibility, rollout eligibility, and known reasoning metadata. Production publishes only the harness/account intersection; the optional QE scope is then applied with omitted meaning unrestricted and `[]` meaning deny-all. Authentication is independent of eligible-model count. The normalized account catalog is keyed by a SHA-256 account scope and cached for at most 60 seconds only after environment verification; missing, stale, malformed, or cross-account state refreshes through the same native Pi request and otherwise fails closed. An explicit provider model-access rejection is redacted to identity/code, invalidates guest and durable eligibility state, fails the frozen Attempt without substitution or another turn, and forces metadata-only capability refresh. A same-account provider catalog that still includes the rejected model becomes `provider_catalog_runtime_inconsistency`, not a hardcoded model denial.
