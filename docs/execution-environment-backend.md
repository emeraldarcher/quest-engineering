...
Phase 1 introduced the Worker-internal execution-environment contract, deterministic test backend, conformance suite, and an un-wired host-native compatibility adapter. Phase 2 added the durable `SbxExecutionEnvironmentBackend`; Phase 3 added the Worker-local private-Git layer described in [Private Git inside Run-owned execution environments](private-git-sbx.md). Phase 4 wires **Pi only** to one Run-owned SBX environment. Worker Protocol v8, Product/PostgreSQL state, Delivery authorization, and Antigravity execution remain unchanged.
