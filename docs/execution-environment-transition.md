...
This document records the preservation boundary and phased transition. Phases 1–3 introduced the execution-environment contract, durable SBX backend, and private Git. Phase 4 wired Pi to the Run-owned boundary. Phase 5 adds Antigravity to the same architecture through the composable `qe-coding-execution-v1` profile. One immutable Run VM may execute Pi and Antigravity; harness changes do not create a second VM. The historical `qe-pi-execution-v2` profile remains byte-for-byte available. No phase authorizes cleanup of retained Run worktrees, Worker state, Herdr sessions, or databases.

The original clean implementation baseline remains commit `430908ecf216f796ea2e038615e9b21c00f71a53` on `preserve/generic-qe-post-multiharness`. No preservation/reference branch is merged. See [execution environment backend architecture](execution-environment-backend.md) for the generic contract and [Antigravity in the Run-owned SBX architecture](antigravity-sbx.md) for current decisions and the 51-point Phase-5 gate.
...
Phase 5 keeps native authenticated model/effort discovery but removes autonomous host execution. Antigravity gets a lineage-private guest HOME, pinned guest executable, static profile-owned QE MCP registration, guest Stop hook, guest log relay, and native conversation identity. Host Antigravity owns OAuth refresh; the VM stores only an invalid account marker, disabled refresh sentinel, and provider-host-scoped access placeholder. The real access token is resolved into SBX proxy memory only. Runtime egress excludes installer and updater endpoints.

The outer SBX filesystem is operation authority. Antigravity launches with autonomous guest permissions so its prompt loop cannot weaken deterministic read-only/read-write enforcement; the optional inner sandbox is disabled to avoid conflicting isolation models. Host-native permission parsing, host state scraping, `.git` mounts, `unbash`, and adapter path-containment logic remain rejected. The unavailable archive ref was not imported; the preservation classification is: generic discovery/control/lifecycle **KEEP**, host launch/config/state policy **REMOVE**, guest bridge/hook/readiness **REWORK**, and old manual probes **DIAGNOSTIC ONLY**.
