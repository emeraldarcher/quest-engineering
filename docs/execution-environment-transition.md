...
One separately authorized paid Attempt then reached the exact guest but its first provider request rejected bundled `gpt-5.3-codex-spark` for the ChatGPT account. It stopped with zero tokens and `$0.00`; its failed execution and retained session remain immutable. The corrective model authority is now `Pi harness catalog ∩ live account API/visibility/rollout eligibility ∩ optional QE scope`, followed by reasoning/tool/profile filtering. A live metadata-only query returned zero account-eligible models, so no Pi executor is currently schedulable. Explicit model-access rejection invalidates account-scoped metadata, fails the frozen Attempt without another turn, and treats a contradictory refreshed catalog as an error rather than a hardcoded denylist. Guest lifecycle writes are serialized and relay teardown drains final idle, keeping terminal failure, retained attachment, and native activity distinct. No further paid Attempt is authorized. Existing Worker `max_concurrency` remains the admission bound and exact one-environment-per-Run ownership is enforced by the durable backend.
