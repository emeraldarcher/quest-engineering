...
test("sequential Implement to Review replaces Builder with Reviewer at the dock", () => {
  ...
  expect(system.actor(builder.actorId)).toBeNull();
  expect(system.actor(reviewer.actorId)?.state).toBe("spawn");
...
test("Parallel actors reconcile independently", () => {
  ...
  expect(system.actor(first.actorId)).toBeNull();
