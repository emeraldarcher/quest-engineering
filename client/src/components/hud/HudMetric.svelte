...
<style>
.hud-metric { display:flex; align-items:center; gap:.45rem; min-width:0; padding:.2rem 1rem; color:#fff1cf; border-left:1px solid #c292626b; white-space:nowrap; }
.icon-slot { display:grid; place-items:center; flex:0 0 1.55rem; width:1.55rem; height:1.55rem; }
.icon-slot img { display:block; max-width:1.45rem; max-height:1.45rem; image-rendering:pixelated; filter:drop-shadow(1px 1px #2c201a); }
.symbol { color:#e6c886; font-size:1.35rem; line-height:1; text-shadow:1px 1px #2c201a; }
.visible-value { display:flex; align-items:baseline; gap:.35rem; }
.visible-value strong { min-width:1ch; color:#fff8e3; font:800 1.2rem Georgia,serif; font-variant-numeric:tabular-nums; }
.metric-label { color:#f0dbb5; font-size:.95rem; font-weight:780; letter-spacing:.015em; }
...
@media(max-width:980px){.hud-metric{padding-inline:.62rem}.metric-label{display:none}.visible-value strong{font-size:1.1rem}}
