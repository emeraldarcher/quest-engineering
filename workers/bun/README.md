...
Pi's bundled `openai-codex` registry is only the harness-supported catalog; it is not ChatGPT account entitlement. Inside the execution profile, QE authenticates the native `originator: pi` models-resource request and publishes only `harness catalog ∩ live account API/visibility/rollout eligibility`. Reasoning values are intersected independently. Omit `QE_EXECUTOR_MODELS` for no additional QE scope. A present empty value is explicit deny-all; a nonempty comma-separated value is an exact `provider/model` allowlist over the eligible intersection. Authentication remains true when a valid account returns zero eligible models, but no executor is schedulable. Unknown metadata fails closed, and no missing model is substituted.
...
Prompt lifecycle is durable and separate from Herdr's terminal-state heuristic: `prompt_intent` is recorded before transport, `prompt_accepted` only after submission acknowledgement, `waiting_for_activity` follows while native evidence is absent, `working` requires the harness's own append-only evidence, `blocked` requires structured attention, and completion or terminal failure settles the turn. Pi uses the exact post-baseline guest lifecycle state; Antigravity uses its post-baseline native conversation handoff log. Herdr `idle`, `working`, terminal text, and process presence do not independently prove semantic activity. The Pi relay serializes guest publications and drains the final `agent_settled` idle report before teardown, so a failed retained, attachable pane cannot remain projected as active work. Herdr's five-second `agent_prompt_stalled` response proves submission, not semantic failure, so adapters call prompt submission without an `until` gate and continue observation. `QE_PROMPT_ACTIVITY_STALL_MS` only changes when waiting is visibly labeled `stalled`; it never fails the Attempt, authorizes a retry, or converts the turn to `uncertain`. Only an unresolved transport outcome at the submission boundary becomes `agent_prompt_uncertain`. Exact-lineage recovery never repeats an accepted prompt.
...
The Herdr startup stress gate creates no execution agents and must report zero prompts. The SBX/Herdr gate first performs authenticated account-eligibility discovery. If the intersection is empty it proves zero schedulable models and performs no launch; otherwise its launch-only section starts and attests guest Pi, proves restart adoption/attachment/private Docker/shell restoration and a hostile host-`pi` PATH trap. Both outcomes require `prompts: 0` and `providerCycles: 0`. The real-provider gates require existing Herdr/Pi subscription authentication and are never run with the fake provider.
