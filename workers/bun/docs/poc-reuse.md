...
| `packages/executor-herdr/src/executor.ts` execution lifecycle | `workers/bun/src/providers/pi/provider.ts` and `src/dispatch/executor.ts` | Behavior retained; code rewritten | The POC class mixed Herdr, Pi, product orchestration, workspace, and fixed result contracts. Production separates TerminalSessionBackend, AgentHarness, and durable dispatch lifecycle. |
